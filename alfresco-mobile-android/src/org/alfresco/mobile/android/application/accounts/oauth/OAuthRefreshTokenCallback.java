...
public class OAuthRefreshTokenCallback implements LoaderCallbacks<LoaderResult<OAuthData>>
{
    ...
    public Loader<LoaderResult<OAuthData>> onCreateLoader(final int id, Bundle args)
    {
        ...
        WaitingDialogFragment.newInstance(R.string.app_name, R.string.error_session_refresh, true)
                .show(activity.getFragmentManager(), WaitingDialogFragment.TAG);
    ...
    private void saveNewOauthData(LoaderResult<OAuthData> loader)
    {
        ...
        case Account.TYPE_ALFRESCO_TEST_OAUTH:
        case Account.TYPE_ALFRESCO_CLOUD:
            acc = AccountManager.update(appContext, acc.getId(), acc.getDescription(), acc.getUrl(), acc.getUsername(),
                    acc.getPassword(), acc.getRepositoryId(), acc.getTypeId(), null, loader.getData().getAccessToken(),
                    loader.getData().getRefreshToken(), acc.getIsPaidAccount() ? 1 : 0, acc.getProtocolType());
            break;
        default:
            break;
        }
        ...
        LocalBroadcastManager.getInstance(appContext).sendBroadcast(
                new Intent(IntentIntegrator.ACTION_LOAD_ACCOUNT_COMPLETED)
                        .putExtra(IntentIntegrator.EXTRA_ACCOUNT_ID, acc.getId()));
    }
