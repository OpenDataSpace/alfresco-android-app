...
public abstract class AbstractOperationThread<T> extends Thread implements Operation<T>
{
    ...
    public AbstractOperationThread(Context context, OperationRequest request)
    {
        ...
        this.repoType = request.getRepoType();
    }
    ...
    protected void saveStatus(int status)
    {
        if (request.getNotificationUri() != null)
        {
            context.getContentResolver()
                    .update(request.getNotificationUri(), request.createContentValues(status), null, null);
        }
    }
    ...
    {
        final AlfrescoSession session;

        if (ApplicationManager.getInstance(context).hasSession(accountId))
        {
            session = ApplicationManager.getInstance(context).getSession(accountId);
        }
        else
        {
        ...
        {
            OdsRepositorySession ods = (OdsRepositorySession) session;
            return ods.getByType(repoType);
        }
